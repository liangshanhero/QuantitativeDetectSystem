...
import com.example.quantitativedetectsystem.Activity.FunctionFormulaActivity;
import com.example.quantitativedetectsystem.domain.Archive;
import com.example.quantitativedetectsystem.domain.Rule;
import com.example.quantitativedetectsystem.service.FunctionService;
...
public class RuleCurve extends BaseCoordinate {
    ...
    public void formula(Canvas canvas){
        ...
        for(int i = 0;i < archive1.length();i++){
            ...
            float x1 = (float)(archive1.getFeature(i).getConc()-min)/(float)(max-min)*(wid-2*pad);
            ...
            if(FunctionFormulaActivity.ONE_TWO == FunctionFormulaActivity.TWO){
                float x2 = (float)(archive2.getFeature(i).getConc()-min)/(float)(max-min)*(wid-2*pad);
